...
class Api(object):
    ...
    def upload(self, f):
        try:
            ...
            rows = s.split('\n')
            del rows[-1]
            data = [np.array([ '"{}"'.format(x) for x in list(csv.reader([row], delimiter=',', quotechar='"'))[0] ]) for row in rows]
    ...
    def headers(self):
        try:
            ...
            for index in range(len(list(self.__class__.df.columns))):
                tooltips.append([])
                tooltips[index] = (list(self.__class__.df[list(self.__class__.df.columns)[index]].iloc[np.arange(tooltips_length)]))
            ...
            empty_columns = list(set(self.__class__.df.columns).symmetric_difference(set(filtered_data.columns)))
    ...
    def create_output(self, headers):
        try:
            d = pd.DataFrame(np.empty((self.__class__.df.shape[0], len(self.__class__.classification_columns)), dtype = np.str), columns=self.__class__.classification_columns)
            self.__class__.output_df = pd.concat([self.__class__.df[headers], d], axis=1, sort=False)
        except Exception as e:
            print(e)
            sys.stdout.flush()
            return e
    ...
    def single_tokens(self, headers):
        try:
            ...
            empty_array[:] = ''
            data = np.column_stack((tex.vocab_,empty_array, empty_array, empty_array,tex.scores_))
            self.__class__.vocab_single_df = pd.DataFrame(data=data, columns=self.__class__.vocab_columns)
            return tokens
        except Exception as e:
            print(e)
            sys.stdout.flush()
    ...
    def update_output_file(self, token):
        try:
            ...
            self.__class__.output_df.loc[:, self.__class__.classification_columns] = self.__class__.output_df.loc[:, self.__class__.classification_columns].applymap(lambda x: x.replace(token['alias'], ''))
            for index in indexes:
                    if token['classification'] ['label']:
                        if self.__class__.output_df.iloc[index][token['classification'] ['label']] and self.__class__.output_df.iloc[index][token['classification'] ['label']] != token['alias']:
                            self.__class__.output_df.set_value(index, token['classification']['label'],self.__class__.output_df.iloc[index][token['classification'] ['label']] + "," + token['alias'])
                        else:
                            self.__class__.output_df.set_value(index, token['classification']['label'],token['alias'])                    
                    else:
    ...
    def update_data(self, token):
        try:
            ...
            #2. Update vocab
            if (' ' in token['label']):
                self.__class__.vocab_multi_df = self.update_vocab(self.__class__.vocab_multi_df, token) 
            else:
                self.__class__.vocab_single_df = self.update_vocab(self.__class__.vocab_single_df, token)
        except Exception as e:
            print(e)
            sys.stdout.flush()
            return e
